...
class AuthorViewSetTestCase(APITestCase):
    ...
    def test_list_authors(self):
        ...
        url = reverse('api:authors-viewset-list')  # Получаем URL ссылку
    ...
    def test_retrieve_author(self):
        ...
        print("______________________________")
        url = reverse('api:authors-viewset-detail', kwargs={'pk': self.author1.pk})  # Укажите имя URL-шаблона и параметры
        print(f"Проверяемы маршрут: {url}")
        response = self.client.get(url)
        print(f"Ответ от сервера: {response.status_code}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_author(self):
        ...
        print("______________________________")
        url = reverse('api:authors-viewset-detail', kwargs={'pk': self.author1.pk})
        print(f"Проверяемы маршрут: {url}")
